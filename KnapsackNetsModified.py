...
class SafetyNet(nn.Module):
    def __init__(self, nKnapsackCategories, nThresholds, starting_thresholds, nineq=1, neq=0, eps=1e-8, 
                 cancel_rate_target=.05, cancel_rate_evaluation=.05, accept_rate_target=.75, accept_rate_evaluation=.75,
    ...
    def forward(self,category,inv_count,price,cancel,collection_thresholds):
        ...
        # 1. inventory_distribution_raw_est -> inventory_distribution_batch_by_threshold_est
        # 2. belief_cancel_rate_cXt_opt -> belief_fill_rate_cXt_opt 
        ...
        # 3. belief_total_demand_cXt_est - realized demand for each category
        ...
        if self.parametric_knapsack:
            ...
            self.knapsack_cancels_matrix = torch.div(
                torch.sum(self.belief_total_cancels_cXt_opt,dim=1).unsqueeze(1).expand_as(self.belief_total_cancels_cXt_opt) -torch.cumsum(self.belief_total_cancels_cXt_opt,dim=1)+self.belief_total_cancels_cXt_opt,
                self.belief_total_demand_opt.expand(self.nKnapsackCategories,self.nThresholds)
            )
            
            self.knapsack_fills_matrix = torch.div(torch.sum(self.belief_total_fills_cXt_opt,dim=1).unsqueeze(1).expand_as(self.belief_total_fills_cXt_opt)-torch.cumsum(self.belief_total_fills_cXt_opt,dim=1)+self.belief_total_fills_cXt_opt,self.belief_total_demand_opt.expand(self.nKnapsackCategories,self.nThresholds))
        ...
        #not relevant? category is BxC matrix, so summing across dim 0 gets the number of accepted orders per category
        ...
        new_revenue_loss = -(1.0/50000)*(truncated_revenue_estimate_sum+self.batch_revenue_sum)
        ...
        return new_revenue_loss, new_cancel_constraint_loss, new_accept_constraint_loss, arrival_probability_batch_by_threshold, log_arrival_prob, log_cancel_prob, log_category_prob, self.estimated_batch_total_demand, observed_cancel_constraint_loss, observed_accept_constraint_loss, self.lp_infeasible
